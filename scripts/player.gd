...
func look(look_vector):
	look_vector = look_vector/get_tree().root.content_scale_size.y
	look_vector = look_vector * mouse_sensitivity
	
	rotate_y(look_vector.x)
	_camera_pivot.rotate_x(look_vector.y)
	_camera_pivot.rotation.x = clamp(_camera_pivot.rotation.x,deg_to_rad(-89),deg_to_rad(89))
